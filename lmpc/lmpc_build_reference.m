...
delta_ref = atan(wheelbase * kappa_ref);

u_ref = zeros(2, N);

% Longitudinal feedforward from measured steady-state map

for i = 1:N

    v_i = v_ref(i);

    % inverse steady-state map:
    % desired velocity -> required pedal input
    u_ref(1,i) = interp1( ...
        params.vss_table, ...
        params.un_table, ...
        v_i, ...
...
% Respect physical pedal bounds
u_ref(1,:) = min( ...
    max(u_ref(1,:), params.u_min(1)), ...
    params.u_max(1));

%% Steering feedforward
u_ref(2,:) = delta_ref(1:N);
...
refs.v      = double(v_ref);
refs.u_ref  = double(u_ref);
refs.delta  = double(delta_ref);
